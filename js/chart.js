...
/* --------------------------------
   Main chart function
-------------------------------- */
export function plotChart(data, canvasId, stepKm = 4) {
    ...
    /* -------- Elevation gradient -------- */
    const elevationGradient = ctx.createLinearGradient(
        0,
        0,
        0,
        ctx.canvas.height
    ...
    elevationGradient.addColorStop(0, 'rgba(27, 206, 120, 0.35)');
    elevationGradient.addColorStop(1, 'rgba(27, 206, 120, 0.05)');

    /* -------- Wind types -------- */
    data.ride_weather.forEach((p, i) => {
        if (i === 0) {
            p.windType = 'crosswind';
        } else {
    ...
    for (let i = 0; i < blockCount; i++) {
        const start = i * stepKm;
        const end = Math.min(start + stepKm, totalDistance);
    ...
    window.chart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    labels: {
                        filter: item => item.datasetId !== 'weather'
                    }
                }
            },
            scales: {
